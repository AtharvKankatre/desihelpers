...
const CCopyLinkButton: FC<CCopyLinkButtonProps> = ({ id }) => {
  ...
  const handleCopyLink = async () => {
    ...
    if (!isActive) {
      toast.warning("Please login to view poster details.");
      router.push("/Login");
      return;
    }
    ...
    const generatedFullPath = `${process.env.NEXT_PUBLIC_Base_API_URL}jobs/${id}`;
    setFullPath(generatedFullPath);

    try {
      await navigator.clipboard.writeText(generatedFullPath);
      if (isMobile) {
        setShowModal(true);
      } else {
        toast.info(
